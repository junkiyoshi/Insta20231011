...
void ofApp::update() {
	...
	for (int i = 0; i < this->triangle_list.size(); i++) {
		...
		auto noise_value = ofNoise(avg.x * 0.001, avg.y * 0.001, avg.z * 0.001 + ofGetFrameNum() * 0.03);
		...
		if (noise_value < 0.5) {
			...
			if (noise_value < 0.4) {
				...
				angle = ofMap(noise_value, 0, 0.4, PI * 10, PI);
			}
			else {

				angle = ofMap(noise_value, 0.4, 0.5, PI, 0);
			}
		...
		for (auto& vertex : vertices) {
			...
			if (noise_value < 0.5) {
				...
				vertex += glm::normalize(avg) * ofMap(noise_value, 0, 0.5, radius * 3, radius);
			}
			else {
